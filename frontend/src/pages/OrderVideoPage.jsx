...
const OrderVideoPage = () => {
	...
	if (isLoading) return <OrderVideoPage />;

	if (loadError || !order) {
		return (
			<PageError
				message="Order not found or you don't have access."
				action={{ to: "/orders", lable: "Back to orders" }}
			/>
		);
	}
				...
				<div className="card-body flex-row items-start gap-4">
					...
					<div>
						...
						<p className="text-sm text-base-content/70">
							Same room as the invite link in chat. Allow camera
							and microphone when your browser asks.
						</p>
					</div>
				</div>
			</div>
